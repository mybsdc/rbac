...
if (!function_exists('p')) {
     ...
     *
     */
    function p($val, $exit = true, $dump = false){

        // 自动获取调试函数名称$print_method
        if ($dump){
            $print_method = 'var_dump';
        } else {
            // $print_method = (is_array($val) || is_object($val)) ? 'print_r' : 'print_r'; // printf输出格式化字符串
            $print_method = 'print_r';
        }

        // 输出到html
        header("Content-type: text/html; charset=utf-8");
        echo '<pre><h2 style="color: #6fa3ef;">debug output:</h2><hr />';
        $print_method($val);
        echo '</pre>';

        // 输出SQL语句
        echo '<pre><h2 style="color: #6fa3ef;">SQL:</h2><hr />';
        $sql = db()->getLastSql();
        echo $sql;
        echo '</pre>';
        if($exit) exit;
